...
\begin{tabularx}{\textwidth}{LL}
  \begin{enumerate}
    ...
    \onslide<1->{\item Pomijamy detale}
    \onslide<1->{\item Korzystamy z niejawnych założeń lub niedowiedzonych twierdzeń}
    \onslide<2->{\item Prowadzimy rozumowanie w konwencji nazw zmiennych Barendregta}
  \end{enumerate}
  &
  \begin{enumerate}
    \onslide<1->{\item Piszemy dla komputera}
    \onslide<1->{\item Musimy obsłużyć wszystkie detale}
    \onslide<1->{\item Korzystamy tylko z jawnych założeń lub i dowiedzonych twierdzeń}
    \onslide<2->{\item ?}
  \end{enumerate}
\end{tabularx}
\end{frame}
...
% \begin{tabularx}{\textwidth}{LL}
  ...
  \onslide<1->{\item Reprezentacja De Bruijna}
  \onslide<1->{\item Higher-Order Abstract Syntax}
  \onslide<1->{\item Techniki nominalne}
  \end{itemize}
  % &
  % {\huge $\lambda x . e$}
% \end{tabularx}
...
\begin{itemize}
  ...
  \\
  \onslide<2->{\item {\color{blue} Stajemy po środku i przedstawiamy wariant logiki nominalnej która oparta jest\\
...
\begin{frame}
  % TODO: w jaki sposób przedstawić zwięźle solver
\includegraphics[width=130mm]{solver}
\end{frame}

\begin{frame}
  % TODO: opowiedzieć, że to takie przeszukiwanie przestrzeni równości atomów
\begin{tikzpicture}[
  level distance=.75in,
  sibling distance=.15in,
  % auto,
...
\frametitle{Indukcja}
  ...
  $\quad (\fForallTerm{\termv} (\fForallTerm{Y} \fCImp{Y \cshlt \termv}{P (Y)}) \fImp {P ({\termv})})$ \\
  ...
  \hspace{-1.25mm}\only<4>{
    $(${\color{red}$\fForallTerm{C}$}$\fForallAtom{\atomv}\fForallTerm{\termv}${\color{red}$\fCImp{\atomv \cfresh {C} }$}$\:P ({\termv}) \fImp P({\tbind{\atomv}{\termv}}))$
  }
  \hspace{-1.25mm}\only<5>{
    $(\fForallAtom{\atomv}\fForallTerm{\termv}
    (${\color{blue}$\fForallTerm{C}\fExistsAtom{\atomv'}\fExistsTerm{\termv'}\fCAnd{\atomv' \cfresh {C} }\fCAnd{\tbind{\atomv}{\termv} \ceq \tbind{\atomv'}{\termv'}}{P ({\termv'})}$}$)
    \fImp P({\tbind{\atomv}{\termv}}))$
  } \\
  $\fImp          (\fForallTerm{\termv} \fCImp{\csymb \termv}{P ({\termv})})$ \\
  $\quad \fImp \fForallTerm{\termv} {P ({\termv})}$ \\
  \end{tabular}
...
%       ; "          ($\forall$ c :term. $\exists$ a' :atom. $\exists$ t' :term. "
