...
let string_of_list' ?(left = "[") ?(sep = "; ") ?(right = "]") string_of_item =
  ...
  | x :: xs ->
    brace $ List.fold_left (fun acc x -> Printf.sprintf "%s%s%s" acc sep $ string_of_item x) (string_of_item x) xs
...
let rec pp_print_swap env fmt =
  pp_print_bracketed
    (fun fmt (a, b) -> pp_print_atom_permuted env fmt a ; print_space fmt () ; pp_print_atom_permuted env fmt b)
    fmt
...
and pp_print_atom_permuted env fmt {perm= pi; symb= a} = pp_print_permutation env fmt pi ; pp_print_atom env fmt a
...
let rec pp_print_term_in_env env fmt = function
  ...
  | T_App (t1, t2) ->
    ...
    print_space fmt () ; pp_print_simple_term_in_env env fmt t2
...
let rec pp_print_kind_in_env env fmt k =
  ...
  | K_Arrow (K_Prop, k2) -> pp_print_string fmt "* ->" ; print_space fmt () ; pp_print_kind_in_env env fmt k2
  | K_Arrow (k1, k2) ->
...
let rec pp_print_formula env fmt formula =
  ...
  let rec pp_print_foralls env = function
    ...
    | F_ForallTerm ((V_Bind (_, x) as x_bind), f) | F_ExistsTerm ((V_Bind (_, x) as x_bind), f) ->
      let env = add_name (var_binder_to_binder x_bind) env in
      pp_print_var env fmt x ; print_space fmt () ; pp_print_string fmt ": term. " ; pp_print_formula env fmt f
    ...
    | F_ForallAtom ((A_Bind (_, a) as a_bind), f) | F_ExistsAtom ((A_Bind (_, a) as a_bind), f) ->
      let env = add_name (atom_binder_to_binder a_bind) env in
      pp_print_atom env fmt a ; print_space fmt () ; pp_print_string fmt ": atom. " ; pp_print_formula env fmt f
    | _ -> failwith ""
  in
  ...
  | F_Impl (f1, f2) -> pp_print_atomic_formula fmt f1 ; space () ; pp_string "=>" ; space () ; pp_print_conclusion f2
  ...
  | F_AppTerm (f, e) -> pp_print_app_left f ; space () ; pp_string "{" ; pp_print_term_in_env env fmt e ; pp_string "}"
  | F_AppAtom (f, a) ->
    pp_print_app_left f ;
    space () ;
    pp_print_parenthesized (pp_print_atom_permuted env) fmt a
  | F_Fix (FV_Bind (fix_name, fix, fix_k), (V_Bind (_, x) as x_bind), k, f) ->
    let env = env |> add_name (var_binder_to_binder x_bind) |> add_name (Bind (fix_name, K_FVar (fix, fix_k))) in
