...
val add_symbol : var -> t -> (t * solver_constr list) option
...
val subst_var : var -> term -> t -> (t * solver_constr list) option
(** [subst_var x t env] performs substitution [x -> t] over all assumptions in [env], returns [None]
...
val set_var_shape : var -> shape -> t -> (t * solver_constr list) option

val add_same_shape : var -> var -> t -> (t * solver_constr list) option
(** [add_same_shape x1 x2 env] adds an assumption [x1 ~ x2] to [env], returns [None] if
...
val add_subshape : shape -> var -> t -> (t * solver_constr list) option
(** [add_subshape t x env] adds an assumption [t < x] to [env], returns [None] if [occurs_check env x t], otherwise [Some env'] *)
