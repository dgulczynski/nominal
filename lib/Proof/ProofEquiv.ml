...
and equiv env1 env2 n1 n2 f1 f2 =
  ...
  | F_Constr c1, F_Constr c2 -> (c1 =:=:= c2) env1 env2
  | F_Constr _, _ -> false
  | F_ConstrImpl (c1, f1'), F_ConstrImpl (c2, f2') | F_ConstrAnd (c1, f1'), F_ConstrAnd (c2, f2') ->
      (c1 =:=:= c2) env1 env2 && equiv (add_constr c1 env1) (add_constr c1 env1) n1 n2 f1' f2'
  ...
  | F_AppAtom (f1, a1), F_AppAtom (f2, a2) ->
      (* we are after computeWNHF so we don't do any substitutions *)
      (T_Atom a1 =:= T_Atom a2) env1 env2 && f1 === f2
  | F_AppTerm (f1, t1), F_AppTerm (f2, t2) ->
      (* we are after computeWNHF so we don't do any substitutions *)
      (t1 =:= t2) env1 env2 && f1 === f2
...
and ( === ) f1 f2 env = equiv env env equiv_depth equiv_depth f1 f2
