...
let ind_weak =
  ...
  |> generalize "t" %> by_induction "t'" "IH" %> inverse_term "t"
  |> intros' ["H"; "a"] %> apply_assm_spec "Hatom" ["a"]
  |> intros' ["H"; "a"; "e"] %> apply_assm_spec "Hlam" ["a"; "e"] %> apply_assm_spec "IH" ["e"] %> solve
  |> intros' ["H"; "e1"; "e2"]
...
let ind_strong' =
  ...
  |> apply_thm_spec ind_strong ["fun _ :term -> P"; "context"; "t"]
  |> intros ["c"] %> assumption
