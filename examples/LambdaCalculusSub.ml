...
let subst_exists =
  ...
  |> intros' ["Hb"; "b"; ""]
     %> compare_atoms "a" "b" %> destr_intro %> exists "v" %> case "var_same" %> repeat by_solver
     %> destr_intro %> exists "b" %> case "var_diff" %> exists "b" %> repeat by_solver
  |> intros' ["Hlam"; "b"; "e_b"; ""]
     %> get_fresh_atom "c" "a v e_b"
     %> add_assumption_parse "He_c" "exists e_c:term. Sub {[b c]e_b} a v e_c"
     %> destruct_assm' "He_c" ["e_c"] %> exists "lam (c.e_c)" %> case "lam"
  ...
  |> intros' ["Happ"; "e1"; "e2"; ""; ""]
     ...
     %> destruct_assm "He2" %> exists "app e1' e2'" %> case "app" %> exists "e1" %> exists "e2"
     %> exists "e1'" %> exists "e2'" %> by_solver %> by_solver %> destruct_goal %> apply_assm "He1"
     %> apply_assm "He2" %> apply_assm_specialized "IH" ["e2"] %> by_solver %> apply_assm "Happ_2"
