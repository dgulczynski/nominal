...
let env_inclusion_cons_both =
  proof' env_inclusion_cons_both_thm |> repeat intro |> intros ["Hinclusion"]
...
let weakening_lemma =
  proof' weakening_lemma_thm |> by_induction "e0" "IH" |> repeat intro
  |> intros ["Htyp"; "Hinc"] %> destruct_assm "Htyp"
  |> intros' ["Hb"; "b"; ""]
     %> case "var" %> exists "b" %> by_solver
  ...
  |> intros' ["Happ"; "e1"; "e2"; "t2"; ""; ""]
     ...
     %> by_solver %> assumption %> assumption
...
let env_inclusion_shuffle =
  ...
  |> intros' ["Hcurrent"; "envc"; ""]
     ...
     %> by_solver %> by_solver %> case "current" %> exists "env" %> by_solver
  |> intros' ["Hnext"; "d"; "td"; "envd"; ""; ""; ""]
...
let typing_env_shuffle =
  proof' typing_env_shuffle_thm |> repeat intro
  |> intros ["H"]
     %> apply_thm_specialized weakening_lemma
          ["e"; "cons a ta (cons b tb env)"; "t"; "cons b tb (cons a ta env)"]
     %> apply_assm "H"
...
let cons_fresh_typing =
  ...
  |> intros' ["Hb"; "b"; ""]
     %> case "var" %> exists "b" %> by_solver %> case "next"
     %> exists' ["a"; "ta"; "env"]
     %> by_solver %> by_solver %> apply_assm "Hb"
  |> intros' ["Hlam"; "b"; "e_b"; "t1"; "t2"; ""; ""; ""]
     %> case "lam" %> compare_atoms "a" "b" %> destr_intro
     %> exists' ["b"; "e_b"; "t1"; "t2"]
     %> by_solver %> by_solver %> destruct_goal %> apply_assm "Hlam_1"
     ...
     %> by_solver %> by_solver %> destruct_goal %> apply_assm "Hlam_1"
     %> apply_thm_specialized weakening_lemma
          ["e_b"; "cons a ta (cons b t1 env)"; "t2"; "cons b t1 (cons a ta env)"]
     %> apply_assm_specialized "IH" ["e_b"; "cons b t1 env"; "t2"; "a"; "ta"]
     %> by_solver %> by_solver %> apply_assm "Hlam_2"
  ...
  |> intros' ["Happ"; "e1"; "e2"; "t2"; ""; ""]
     ...
     %> by_solver %> by_solver %> apply_assm "Happ_1"
     %> apply_assm_specialized "IH" ["e2"; "env"; "t2"; "a"; "ta"]
     %> by_solver %> by_solver %> apply_assm "Happ_2"
  |> qed
...
let env_inclusion_double_skip =
  ...
  |> intros' ["Hdc"; "envc"]
     ...
     %> repeat by_solver %> case "next"
     %> exists' ["b"; "tb"; "cons c tc env2"]
     %> repeat by_solver %> case "current" %> exists "env2" %> by_solver
  |> intros' ["Hnext"; "c'"; "tc'"; "envc'"; ""; ""; ""]
  ...
  |> intros' ["Hdb"; "envb"]
     %> case "next"
     %> exists' ["a"; "ta"; "cons b tb (cons c tc env2)"]
     %> repeat by_solver %> case "current" %> exists "cons c tc env2" %> by_solver
  |> intros' ["Hd"; "b'"; "tb'"; "env1'"; ""; ""]
     %> case "next"
     %> exists' ["a"; "ta"; "cons b tb (cons c tc env2)"]
     %> repeat by_solver %> case "next"
...
let env_inclusion_double_skip' =
  ...
  |> intros' ["Hda"; "enva"]
     ...
     %> repeat by_solver %> case "current" %> exists "cons b tb env2" %> by_solver
  |> intros' ["Hnext"; "a'"; "ta'"; "enva'"; ""; ""; ""]
  |> intros' ["Hdb"; "envb"]
     %> case "next"
     %> exists' ["c"; "tc"; "cons a ta (cons b tb env2)"]
...
let swap_lambda_typing_lemma =
  ...
  |> intros' ["Hc"; "c"; ""; ""]
     %> intros' ["Hcurr"; "env'"]
     %> case "var" %> exists "b" %> by_solver %> case "current" %> exists "cons a tb env" %> by_solver
     %> intros' ["Hdiff"; "d"; "td"; "env'"; ""; ""; ""]
     %> intros' ["Hcurr"; "env''"]
     %> case "var" %> exists "a" %> by_solver %> case "next"
     %> exists' ["b"; "ta"; "cons a tb env"]
     %> by_solver %> by_solver %> case "current" %> exists "env" %> by_solver
     %> intros' ["Hdiff"; "f"; "tf"; "env''"; ""; ""]
     %> case "var" %> exists "c" %> by_solver %> case "next"
     %> exists' ["b"; "ta"; "cons a tb env"]
     %> repeat by_solver %> case "next"
     %> exists' ["a"; "tb"; "env''"]
     %> repeat by_solver %> assumption
  ...
  |> destr_intro %> case "lam"
     ...
     %> repeat by_solver %> destruct_goal %> apply_assm "Hlam_1"
     ...
     %> apply_assm "Hlam_2" %> compute
     %> intros ["d"; "td"]
     %> destr_intro
     %> intros' ["Hcurr"; "envd"]
     %> case "current" %> exists "cons a ta (cons a tb env)" %> by_solver
     %> intros' ["Hnext"; "f"; "tf"; "envf"; ""; ""; ""]
     %> intros' ["Hcurr"; "envd"]
     %> case "next"
     %> exists' ["b"; "t1"; "cons a ta (cons a tb env)"]
     %> repeat by_solver %> case "current" %> exists "cons a tb env" %> by_solver
     %> intros' ["Hnext"; "g"; "tg"; "envg"; ""; ""]
     %> case "next"
     %> exists' ["b"; "t1"; "cons a ta (cons a tb env)"]
     %> repeat by_solver %> case "next"
     %> exists' ["a"; "ta"; "cons a tb env"]
     %> by_solver %> by_solver %> destruct_assm "Hnext"
     %> intros' ["contra"; "_env"]
     %> discriminate
     %> intros' ["Hcurr"; "h"; "th"; "envh"; ""; ""]
     %> case "next"
     %> exists' ["a"; "tb"; "envh"]
     %> by_solver %> by_solver %> apply_assm "Hcurr"
  |> destr_intro %> case "lam"
  ...
  |> intros' ["Happ"; "e1"; "e2"; "t2"; ""; ""]
     ...
     %> by_solver %> apply_assm "Happ_1"
     %> apply_assm_specialized "IH" ["e2"; "env"; "t2"; "a"; "ta"; "b"; "tb"]
     %> by_solver %> apply_assm "Happ_2"
  |> qed
...
let swap_lambda_typing =
  ...
  |> intros' ["Hc"; "c"; ""; ""]
     %> intros' ["Heq"; "env'"]
     %> case "var" %> exists "b" %> by_solver %> case "current" %> exists "env" %> by_solver
     %> intros' ["Hdiff"; "d"; "td"; "env'"; ""; ""]
     %> case "var" %> exists "c" %> by_solver %> case "next"
     %> exists' ["b"; "t'"; "env'"]
     %> by_solver %> by_solver %> apply_assm "Hdiff"
  ...
  |> destr_intro %> case "lam"
     ...
     %> repeat by_solver %> destruct_goal %> assumption
  |> apply_thm_specialized weakening_lemma
       ["[a b]e_c"; "cons b t' (cons c t1 env)"; "t2"; "cons c t1 (cons b t' env)"]
  |> apply_assm_specialized "IH" ["e_c"; "cons c t1 env"; "t2"; "a"; "b"; "t'"] %> repeat by_solver
  |> apply_thm_specialized weakening_lemma
       ["e_c"; "cons c t1 (cons a t' env)"; "t2"; "cons a t' (cons c t1 env)"]
     %> apply_assm "Hlam_2"
