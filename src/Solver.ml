...
module Solver = struct
  ...
  let rec solve_fresh env assmpts a e =
    ...
    | Atom {perm; symb= b} -> (
      ...
      | Some (swap, pi) ->
          solve_fresh_with_swap env assmpts a swap $ Atom {perm= pi; symb= b} )
    | Var {perm; symb= x}  -> (
      match outer_swap perm with
      | None            -> SolverEnv.is_fresh env a x
      | Some (swap, pi) ->
          solve_fresh_with_swap env assmpts a swap $ Var {perm= pi; symb= x} )
    | Lam (alpha, t)       -> solve_ env ((a =/=: alpha) :: assmpts) a #: t
    | App (t1, t2)         ->
        solve_fresh env assmpts a t1 && solve_fresh env assmpts a t2
    | Fun _                -> true

  and solve_fresh_with_swap env assmpts a (alpha1, alpha2) e =
  ...
  and solve_eq env assmpts e1 e2 =
    ...
    | Atom {perm= []; symb= a}, Atom {perm; symb= b} -> (
      match outer_swap perm with
      | None            -> a = b
      | Some (swap, pi) -> solve_eq_with_swap env assmpts a swap $ Atom {perm= pi; symb= b}
      )
  ...
  and solve_eq_with_swap env assmpts a (alpha1, alpha2) e =
  ...
  and solve_assmpt_fresh env assmpts goal a = function
    | Atom {perm= pi; symb= b} ->
        ...
        solve_assmpt_fresh_with_swap env assmpts a swap (Atom {perm= pi'; symb= b}) goal
    | Var {perm= pi; symb= x}  -> (
      match outer_swap pi with
      | None             -> solve_ (SolverEnv.add_fresh env a x) assmpts goal
      | Some (swap, pi') ->
          solve_assmpt_fresh_with_swap env assmpts a swap (Var {perm= pi'; symb= x}) goal
      )
    | Lam (alpha, t)           -> solve_ env ((a =/=: alpha) :: (a #: t) :: assmpts) goal
    | App (t1, t2)             -> solve_ env ((a #: t1) :: (a #: t2) :: assmpts) goal
    | Fun _                    -> solve_ env assmpts goal

  and solve_assmpt_fresh_with_swap env assmpts a (alpha1, alpha2) e goal =
  ...
  and solve_assmpt_eq env assmpts goal t1 t2 =
    ...
    | Atom {perm= []; symb= a}, Atom {perm= pi; symb= b} -> (
      ...
      | Some (swap, pi') ->
          solve_assmpt_eq_with_swap env assmpts a swap (Atom {perm= pi'; symb= b}) goal )
    | Atom {perm= pi; symb= a}, Atom beta ->
        solve_assmpt_eq env assmpts goal
        $ Atom {perm= []; symb= a}
  ...
  and solve_assmpt_eq_with_swap env assmpts a (alpha1, alpha2) e goal =
    solve_ env
      ((Atom (pure a) =: e) :: (a =/=: alpha1) :: (a =/=: alpha2) :: assmpts)
      goal
    && solve_ env ((Atom alpha2 =: e) :: (a ==: alpha1) :: assmpts) goal
    && solve_ env
         ((Atom alpha1 =: e) :: (a =/=: alpha1) :: (a ==: alpha2) :: assmpts)
         goal
