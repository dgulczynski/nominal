...
module Solver = struct
  let rec solve_fresh env a e =
    ...
    | Atom {perm= (alpha1, alpha2) :: pi; symb= b} ->
        ...
        solve_fresh env a $ Atom {perm= pi; symb= b}
        && solve_fresh env a $ Atom (permute pi alpha2)
        && solve_fresh env a $ Atom (permute pi alpha1)
    | Var {perm= []; symb= x} -> SolverEnv.is_fresh env a x
    | Var {perm= (_alpha1, _alpha2) :: pi; symb= x} ->
        (* TODO: add assumptions to env *)
        solve_fresh env a $ Var {perm= pi; symb= x}
    | Lam ({perm= []; symb= b}, _) when a = b -> true
    | Lam ({perm= []; symb= b}, t) -> (
      (* TODO: merge with pattern below *)
  ...
  and solve_eq env e1 e2 =
    ...
    | Atom {perm= []; symb= a}, Atom {perm= _pi; symb= b} ->
        (* TODO: add assumptions to env *)
        solve_eq env $ Atom {perm= []; symb= a} $ Atom {perm= []; symb= b}
    | Atom {perm= pi; symb= a}, Atom b ->
        solve_eq env $ Atom {perm= []; symb= a} $ Atom (permute pi b)
    ...
    | Var {perm= []; symb= x}, Var {perm= pi; symb= x'} when x = x' ->
        let test ({perm= pi; symb= a} as alpha) =
          ...
          || solve_fresh env a $ Var {perm= reverse pi; symb= x}
        in
        List.for_all test $ free_vars_of pi
    | Var {perm= pi; symb= x}, Var x' ->
        solve_eq env $ Var {perm= []; symb= x} $ Var (permute pi x')
  ...
  and solve_with_env env = function
