...
module SolverEnv = struct
  ...
  let add_to_assumptions {gamma; assumptions} a = {gamma; assumptions= a :: assumptions}

  let add_fresh env a x = add_to_gamma env $ AFresh (a, x)

  let add_neq env a a' = if a = a' then None else Some (add_to_gamma env $ ANeq (a, a'))

  let add_assumption env a =
    (* TODO: implement it properly *) Some (add_to_assumptions env a)
