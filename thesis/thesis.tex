...
\end{figure}
Otherwise we can just substitute the name for the expression,
and while substitution over the environment $\cEnv$ and goal $\sconstr$ is indeed the simplest substitution, substituting in $\icEnv$ environment is a more involved process
...
\begin{figure}[htpb]
  ...
  \framebox[\textwidth]{
    \begin{tabularx}{\textwidth}{CCCr}
      \\ \multicolumn{3}{c}{ $
      \inference{
        \atomexp_1 \cfresh \tbind{\atomexp_2} \term_2,\; \term_1
        = \permswap{\atomexp_1}{\atomexp_2}\term_2 , \; \cEnv ; \icEnv \solverRule \sconstr
      }{
      ...
      \inference{
        ...
        \atomv \ceq \atomv , \cEnv ; \icEnv \solverRule \sconstr
      } $ & $
      \inference{
        \cEnv ; \icEnv \solverRule \sconstr
      }{
        \termv \ceq \termv , \cEnv ; \icEnv \solverRule \sconstr
      } $ & \textsc{EqRefl}
    \end{tabularx}
  }
  \caption{Equality assumption rules}
...
\end{tabularx}
  ...
  \caption{Interpretation of constraints in $\icEnv$}
  \label{fig:irreducible-interpretation}
\end{figure}
