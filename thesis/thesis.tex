...
Justifiably, we skip over this detail{\mdash}however, the induction principle obliges us to prove the case for arbitrary variable names.

% \section{Problem statement}
Addressing this gap in formal reasoning requires careful considerations to come up with a resolution.
Fortunately, there exist some solutions to that problem{\mdash}and one particular
...
Additionaly, swapping has one other useful logical properties{\mdash}it is involutive
...
given by the grammar below, where $a$ ranges over an infinite set of names{\mdash}or rather \it{atoms}.
\begin{figure}[htbp]
  \centering
...
$a$ not being a member of the \it{support set} of $t${\mdash}but for our purposes, the simple inductive definition will suffice.}\
...
There's two fundamental observations paved the way to empowering such
framework towards automatic handling of syntactical properties:
\begin{itemize}
\item The structural nature of these predicates and relations allows the deduce of additional assumptions based on existing or assumed ones.
It can also go the other way: one can break down and analyse the assumptions
up until the simplest parts.
\item These properties are intertwined together.
For example, if atom $a$ is fresh in term $t$ and terms $t$ and $t'$ are alpha-equivalent,
then it must hold that $a$ is fresh in $t'$.
\end{itemize}
...
Construction $\tbind{\atomexp} \term$ represents a \it{binder}{\mdash}informally, we think of it as binding the occurences of $\atomexp$ in $\term$,
similarly to a lambda abstraction{\mdash}yet it \it{isn't} a binder,
...
describing the relationship between their arguments{\mdash}atoms and terms.
...
TODO: write that terms and atoms can be permuted. and describe how permutation works.
...
TODO: change rules such that $\cEnv$ is empty and write why we show goal-reducing first
while assumption-reducing are actually done beforehand.

TODO: explain that if  $ \perm \atomv \equiv \atomexp$ then $ \atomexp \cfresh \term \equiv \atomv \cfresh \perm^{-1} \term$
...
{\mdash}the contradictory branches will resolve through absurd environment.
\begin{figure}[htbp]
    \centering
...
{\mdash}the only way they could be equal is that if that permutation is idempotent on them.
...
TODO: describe the meta-assumption $\forall \atomv \in \pi. \dots$
...
\end{figure}
TODO: describe $|t|$ (shape computation).\\
...
TODO: This chapter is rather technical but the heart of our work,
so make it easier to read by providing some examples.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{Higher Order Logic}
By constructing the Solver, we have built the sound logical system designed for handling and resolving constraints.
We extend its utility and accesibility
...
In subsequent chapters, our focus will shift towards constructing a dedicated proof system for this higher-order logic and a proof assistant.
...
This is exactly the reason of introducing kinds{\mdash}to allow us to use recursive predicates in a safe and sound fashion.
...
\begin{figure}[htbp]
  ...
  \label{fig:deduction}
\end{figure}
TODO: rewrite this so its clear what we are saying here: \\
Notice that in the case of constraint-and-guard, the rule for elimination is restricted
to only formulas of kind $\kProp$.
This is due to the nature of the guard{\mdash}if we want to eliminate it,
...
TODO: describe how quantifiers are true binders (so we can alpha rename!), and remind reader that $\tbind{\atomv}\term$ is not a true binder! and maybe put it earlier.
...
Nonetheless, it's simply that{\mdash}\it{an equivalence relation}{\mdash}and it
...
Quantifiers are handled the same way as function above{\mdash}as they all are a form of bind.
...
{\mdash}and their string \it{names} are kept within the environment and
...
\begin{tabularx}{\textwidth}{|l|L|}
  ...
  \\[1.5ex] \hline
  \lstt{subst}        & Substitutes atoms for atom expressions and variables for terms in goal and environment{\mdash}as long as Solver proves their equality.
  \\[5ex] \hline
...
we arrive at identity{\mdash}but have one less case to consider
...
\begin{lstlisting}[language=OCaml]
  |> intro'
\end{lstlisting}
First one is that \lstt{e} is a~variable {\mdash} which again contradicts with empty enviroment:
\begin{lstlisting}[mathescape,language=OCaml,escapebegin=\color{codegreen}]
  |> intros' ["contra"; "a"; ""]
     %> ex_falso
(* Proof state:
...
*)
     ...
     %> assumption
\end{lstlisting}
Next, \lstt{e} is a~lambda abstraction{\mdash}so a~value.
\begin{lstlisting}[mathescape,language=OCaml,escapebegin=\color{codegreen}]
...
\lstt{e1} must be an abstraction{\mdash}we just need to ensure the Prover that all preconditions are met.
\begin{lstlisting}[mathescape,language=OCaml,escapebegin=\color{codegreen}]
...
{\mdash}which is made possible by swapping atoms while maintaining alpha-equality.
Consequence of that is when we destruct \lstt{Hsub} we get that
\tt{e = lam (c.e\_c)} and \lstt{e' = lam (c.e\_c')}
{\mdash}while \lstt{b.e\_b} and \lstt{c.e\_c} are equal, \lstt{b} and \lstt{c} don't have to be.
