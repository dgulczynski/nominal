...
To demonstrate its potential for reasoning within the programming languages world, we conduct proofs of classical properties of simply typed lambda calculus using this framework.
}
...
over the structure of an expression, where in the case for an abstracion we will implicitly only show
...
by utilising it's own constraint logic programming langauge Elf.
The principal authors of Twelf are Frank Pfenning, and Carsten Schürmann.
Multiple reasearch projects were developed using it, including a type safety proof for Standard ML\cite{twelf-ml}.
\\
...
It encodes HOAS approach using dependent types and provides support for reasoning with context and contextual objects.
It's developed at the Complogic group at McGill University, led by Professor Brigitte Pientka
...
denoted by metavariable $\symb$, drawn from yet another set disjoint with atoms and variables.
...
but a simple syntactical construction glueing together an atom with a term.
...
for when the variable is substituted for a term, we would apply that permutation
on the substituted term.
We also define the term shapes and how to compute them, which
we will use to resolve contraints.
...
However, the notion od applying swaps is included in the
model definition.
...
\end{figure}
With above machinery, we establish the relation $\tmEnv \vDash \constr$
that interprets the constraints in our model, using some mapping $\tmEnv$.
As a consequence of using of De Bruijn indices, we can trivially compute the
...
Morever, we define some syntactic sugar.
\begin{figure}[htbp]
    \centering
...
\end{enumerate}
Notice that it is impossible for any two of these assumption to be valid at the same time\-the
contradictory branches will resolve through absurd environment.

If the left-hand side's term is permuted we move the permutation
to the right-hand side by inverting it.
There's also special check for a variable equal to it's permuteded self\-the
...
we reduce the atom by consdiering the swap cases, analogously to as we did in the
goal-reducing assumptions. On the last page of this section we show a short example describing
how the Solver reduces assumptions.
...
We again most consider the special case where a variable is assumed to be equal to itself with some permutation applied, where we extend the environment $\cEnv$ by a meta-assumption denoted as
$(\forall \atomv \in \perm.\;\atomv = \perm \apperm \atomv \;\vee\;\atomv \cfresh \termv)$.
Solver handles this by creating multiple environments where each atom $\atomv$ occuring in permutation $\perm$ generates an assumption $\atomv = \pi \atomv$ or $\atomv \cfresh \termv$, and every combination of these assumptions is used to create new environments by
...
Equality assumptions reduce to substitution of the name for the expression, where
substituting in $\icEnv$ environment is a more involved process
...
We will see how it decides that
constraints $(a \cneq b,\: a \cneq c,\: b \cfresh \tbind{a}\permswap{b}{c}{E})$
collectively imply the satisfaction of goal constraint $(c \cfresh E)$
by performing a procedure resembling an exhaustive search on atom comparison.
It begins by taking the already irreducible assumptions $(a \cneq b,\: a \cneq c)$ and moving then to $\icEnv$.
Then, it analyzes how $b \cfresh \tbind{a}\permswap{b}{c}{E}$
can hold, branching into two possibilities.
First case is that $b$ and $a$ are equal and quickly leads to contradiction.
The second one is that $b$ is different than $a$ and fresh in $\permswap{b}{c}{E}$.
...
First case is that $b$ is neither $b$ or $c$, contradicting the reflexivity of equality.
Second one is that $b$ is equal to $b$ but different than $c$, so it gets swapped for $c$
in the freshness assumption, which then becomes $c \cfresh E$ and matches with the goal.
Final case is that $b$ is equal to $c$, so the Solver merges these atoms together,
subsituting $b$ to $c$ everywhere.
The freshess assumption turns into $c \cfresh E$ by substitution, so it again matches the goal.
As all branches resolved properly, the whole judgement is proved.
...
denoted \shenv{\shape}.
Veryfying whether a constraint is included in $\icEnv$ can then be accomplished straightforwardly.
...
freshness of an atom in a in a variable is simply acknowledged in the $\Fresh$ mapping.
...
\begin{figure}[htbp]
  ...
  \label{fig:deduction}
\end{figure}
We define \sc{ConstrE} as proof constructor for dealing with a contradictory
...
quantification, and restrict the quantified name to be \it{fresh} in the environment\-it
cannot occur in any assumption.
\begin{figure}[htbp]
  \centering
  \begin{tabularx}{\textwidth}{|C|}
  \hline \\ $
...
then we try to progress with equivelnce by recursing on the structure of formulas.
...
This approach  let's the users of the library pattern match and ``see''
...
Each and every rule of natural deduction described in previous chapter
is implemented by a different function in \tt{Proof},
that are used the the user to directly construct forward proofs,
...
To accomodate for that, we included the \it{top-down} proof structure as the \lstt{incproof} data type.
...
Finally, take a look how the theorem that 1 is a natural number is expressed,
