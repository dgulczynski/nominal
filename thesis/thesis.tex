...
As such proofs are incomplete by nature, they must have \textit{holes},
and live within some \textit{proof context}, as defined in modules \tt{IncProof}.
...
let canonical_form'_thm = lambda_thm
...
We proof preservation through induction on term \tt{e} the case analysis on assumption \tt{Steps e e'}.
\begin{lstlisting}[mathescape, language=OCaml]
let preservation =
  let deduce_app_typing = ...
  in proof' preservation_thm
  |> by_induction "e0" "IH"
...
We use local tactics \tt{deduce\_app\_typing}, \tt{contra\_var} and \tt{contra\_app} not to repeat some boilerplate operations and omit the definition here to encourae the reader to look into the
...
*)
     ...
     (* Typing e2 env t2 $\implies$ Steps e2 e2' $\implies$ Typing e2' env t2*)
       ...
       (* Typing e2 env t2 $\implies$
              Steps e2 e2' $\implies$ Typing e2' env t2 *)
         %> by_solver
         %> apply_assm "Happ_2"
         %> apply_assm "He2_2"
\end{lstlisting}
...
*)
    ...
    (* Typing v env t2 $\implies$ Typing e_a {cons a t2 env} t
           $\implies$ Sub e_a a v e' $\implies$ Typing e' env t *)
    %> apply_assm "Happ_2"
    %> apply_thm_specialized
