...
\begin{figure}[htbp]
    ...
    \begin{tabularx}{\textwidth}{|CC|}
      ...
      \\ & \\ \hline
      \end{tabularx}
  \caption{Constraints' syntax sugaring}
...
% TODO: Why higher order? not 1st, 2nd order
...
Despite these similiartiy,
in the rule for eliminating constraint conjunction guard (\sc{ConstrAndE2}),
we restrict the guarded formulas to be of kind $\kProp$.
If one wants to eliminate the guard to use the inner formula,
one can only do so with formulas that \it{make sense}
(or rather: pass kind checking) on their own,
without the constraint $\constr$ guarding them, as opposed to the kinding rule
where $\constr$ was added to the kinding environment.
% TODO: emphasise that formula needs to kind on its own, (prop is irelevent!) (c must hold in gamma)
