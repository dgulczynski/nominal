...
Terms, constraints, kinds, and formulas are defined in \lstt{Types} module,
...
The \it{Solver} ihabits its own dedicated \lstt{Solver} module along with \lstt{SolverEnv}
...
\lstt{Proof}, \lstt{ProofEnv}, \lstt{ProofEquiv} and is a direct implementation of the proof-theoretic rules.
...
\end{lstlisting}
Note that the \lstt{Proof} modules provide methods for constructing forward proofs, i.e., those in which more complex conclusions are built from simpler, already proven facts.
...
we delegate the responsibility for the correctness of the proofs to the \lstt{Proof} module, and the \lstt{IncProof} module serves as a kind of facade for it.
...
assumptions via\lstt{apply\_in\_assumption} tactic.
One can also add introduce assumptions to the proof context from theorems via \lstt{add\_assumption\_thm}
(specialized if needed via \lstt{add\_assumption\_thm\_spec}) --
or simply add any assumption to the current context together with a new goal (of proving that assumption)
via \lstt{add\_assumption}.
...
Note that the user provides\lstt{destr\_assm'} with a \it{name} that will be bound
...
in previous chapter.
Again argument\lstt{a} to\lstt{get\_fresh\_atom} is given by name and is bound by
a fresh internal identifier automatically.

Additional we provide the user with some auxiliary tactics:
\begin{itemize}
\item \lstt{subst} --- substitutes atoms for atom expreesions
  and variables for terms in goal and environment
  --- as long as Solver proves their equality,
\item \lstt{compute} --- computes WHNF of the current goal,
\item \lstt{try} --- applies a tactic and returns unchanged state if the tactic fails
\item \lstt{repeat} --- applies given tactic (until failure),
\item \lstt{trivial} ---  tries applying some simple tactics
\end{itemize}
Finally, the function \lstt{qed} accepts a prover state and finalizes it.
If the proof state is indeed finished, the function transforms it into a forward proof.
This transformation guarantees correctness through the utilization of straightforward rules embedded within the \lstt{proof} smart constructors.
...
From \lstt{canonical\_form} theorem we know then \lstt{e1} must be an abstraction
