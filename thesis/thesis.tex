...
The ultimate gold of our work is to create a logic for dealing with variable binding,
and there's no better way to use than to prove some things about lambda calculus.

We will take a look on simply typed lambda calculus and examine proofs of
its two major properties of \textit{type soundness}: \textit{progress} and \textit{preservation}.
Before we delve into the proofs, let's first establish needed relations:
...
As one would expec, we will need a lemma about \textit{canonical forms},
which states all values are of \textit{arrow} type and can be \textit{inversed} into an abstraction term.
(because we did not consider any true base type like a \texttt{Bool} or \texttt{Int}).
